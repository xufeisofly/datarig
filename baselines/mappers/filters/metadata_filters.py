...
def language_filter_v2(page: Dict, keep_languages: List[str], key='language_id_whole_page_langdetect', threshold=0.0) -> \
...
def url_substring_filter(banlist: Union[str, List] = None, banlist_from_fname: str = None, ignore_chars: List[str] = None, 
                         num_banned_substrs: int = 1, exact_domain_match: bool=False, match_substrings=True, case_sensitive=False, annotate=False, token="") -> List[Dict]:
    """
    ...
    def filter_fn(page: Dict):
        ...
        if exact_domain_match and url in banlist:
            return set_filter_reason_if_annotate(page, "url_substring_filter"+token, annotate)
