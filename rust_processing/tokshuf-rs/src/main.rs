use crate::io::{count_dirsize, expand_dirs, read_pathbuf_to_mem, write_mem_to_pathbuf};
...
fn get_overflow_basename(overflow_round: usize, overflow_id: usize) -> PathBuf {
    ...
    PathBuf::from(format!(
        "overflow_{:04}_{:08}.tar",
        overflow_round, overflow_id
    ))
}
...
fn build_cellmap(filenames: &Vec<PathBuf>) -> Option<CellMap> {
    ...
    for (idx, filename) in filenames.into_iter().enumerate() {
        ...
        let writer = Arc::new(Mutex::new(Builder::new(BufWriter::new(
            OpenOptions::new()
                .append(true)
                .create(true)
                .mode(0o644)
...
fn write_contexts(
    ...
    while tokens.len() >= seqlen {
        ...
        header.set_cksum();
        let mut builder = local_cell_mapper
            .get(&local_cell_fid)
            .unwrap()
            .lock()
            .unwrap();
        builder
            .append_data(&mut header, uid, compressed_data)
            .unwrap();
    }
    Ok(tokens)
}
...
fn save_manifest(
    manifest_lines: Arc<Mutex<Vec<String>>>,
    output_dir: &PathBuf,
) -> Result<(), Error> {
    ...
    <U as TryFrom<T>>::Error: std::fmt::Debug,
{
    ...
    vec.into_iter()
        .map(|item| U::try_from(item).map_err(|e| format!("Cannot cast element: {:?}", e)))
        .collect()
}
...
fn coarse_shuffle(
    ...
    for (input_idx, input_file) in input_files.iter().enumerate() {
        ...
        threadpool.execute(move || {
            if shuffle_only {
                coarse_shuffle_single_tarfile(&input_file, local_cell_mapper, seed, input_idx)
                    .unwrap();
            } else {
                tokenize_coarse_shuffle_single(
                    &input_file,
                    local_cell_mapper,
                    seed,
                    seqlen,
...
fn tokenize_coarse_shuffle_single(
    input_file: &PathBuf,
    local_cell_mapper: CellMap,
    seed: usize,
    seqlen: usize,
...
fn coarse_shuffle_single_tarfile(
    input_file: &PathBuf,
    local_cell_mapper: CellMap,
    seed: usize,
    input_idx: usize,
...
fn process_local_cell(
    filename: &PathBuf,
    overflow_writer: &Option<CellMap>,
    output_dir: &PathBuf,
    wds_chunk_size: usize,
    wds_chunk_id: &AtomicUsize,
    total_token_count: &Arc<AtomicUsize>,
    seed: usize,
...
fn finalize_chunk(
    ...
    output_dir: &PathBuf,
    wds_chunk_id: &AtomicUsize,
    total_context_count: &Arc<AtomicUsize>,
    manifest_vec: &Arc<Mutex<Vec<String>>>,
) -> Result<()> {
...

    {
        ...
        for (idx, (path, contents)) in chunk.iter().enumerate() {
            ...
            header.set_cksum();
            header.set_mode(0o644);
            let output_context_path =
                PathBuf::from(format!("{:08}_{:08}_{}", chunk_id, idx, path.display()));
            builder
                .append_data(&mut header, output_context_path, &mut contents)
                .unwrap();
        }
...
fn fine_sort_and_save(
    ...
    let mut src_filenames: Vec<PathBuf> = (0..num_local_cells)
        .map(|idx| local_cell_dir.clone().join(get_local_cell_basename(idx)))
        .collect();
    ...
    while src_filenames.len() > 0 {
        ...
        let overflow_filenames: Vec<PathBuf> = (0..num_overflows)
            .map(|idx| {
                local_cell_dir
                    .clone()
                    .join(get_overflow_basename(overflow_round, idx))
            })
            .collect();
        let overflow_writers = build_cellmap(&overflow_filenames);
        println!(
            "STARTING ROUND {:?} | {:?} SRC FILES | {:?} WRITERS",
            overflow_round,
            src_filenames.len(),
            overflow_filenames.len()
        );
...
fn make_exp_data_json(
    ...
    */
    ...
    if exp_data.get("num_tokens").is_none() {
        ...
        println!(
            "Provided token count ({:?}) doesn't match computed: {:?}",
            exp_data["num_tokens"], counted_tokens
        );
    }
...
fn main() -> Result<()> {
    ...
    */
    ...
    let total_token_count = coarse_shuffle(
        &input_files,
        &args.local_cell_dir,
        threads,
        args.num_local_cells,
        args.seqlen,
        args.seed,
        args.shuffle_only,
        &args.tokenizer,
    ...
    let total_context_count = fine_sort_and_save(
        &args.local_cell_dir,
        &args.output,
        args.num_local_cells,
        threads,
    ...
    println!(
        "Processed {:?} tokens",
        total_token_count.fetch_add(0, Ordering::SeqCst)
    );
    println!(
        "Processed {:?} contexts",
        total_context_count.fetch_add(0, Ordering::SeqCst)
    );
    Ok(())
}
